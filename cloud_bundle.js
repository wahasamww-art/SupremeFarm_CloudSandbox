...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    bindEvents() {
        ...
        c.addEventListener('click', (e) => {
            ...
            if (tgt.classList.contains('sf-ps-start-btn')) { this._startOne(tgt.dataset.key); if (this.activeMachineKey === tgt.dataset.key) this._renderMachines(); }
            ...
            else if (tgt.classList.contains('sf-ps-auto-collect-cb')) {
                const sched = this.schedules[tgt.dataset.key];
                if (sched) { sched.autoCollect = tgt.checked; this._saveSchedules(); }
            }
                ...
                const matId = parseInt(tgt.dataset.matid);
                if (matId) this._navigateByMaterialId(matId);
            }
        });
    ...
    _renderMachines() {
        ...
        if (this.activeMachineKey) {
            ...
            <div class="sf-ps-manage-view" data-key="${item.key}" style="background:rgba(52,152,219,0.05);border:1px solid #3498db44;border-radius:8px;padding:12px;box-shadow:inset 0 0 10px rgba(0,0,0,0.5);">
                ...
                <div style="display:flex; align-items:center; justify-content:center; gap:10px; margin-bottom:10px; border-bottom:1px solid #333; padding-bottom:8px;">
                    <label style="color:#aaa; font-size:13px; display:flex; align-items:center; gap:5px; cursor:pointer;" title="إذا تم إيقافه، سيتوقف البوت عن التجميع التلقائي وينتظر تدخلك.">
                        <input type="checkbox" class="sf-ps-auto-collect-cb" data-key="${item.key}" ${sched.autoCollect !== false ? 'checked' : ''}>
                        تجميع تلقائي (Auto-Collect)
                    </label>
                </div>
                            ...
                            const reqHtml = (p.reqMats || []).map(rm => {
                                ...
                                if (producer) {
                                    if (producer.status === 'placed') {
                                        chainHtml = `<button class="sf-ps-nav-source-btn" data-matid="${rm.id}" style="background:#2980b9;border:none;color:#fff;font-size:10px;padding:2px 4px;border-radius:3px;margin-right:4px;cursor:pointer;" title="انقر للذهاب إلى الآلة">⚙️ ${producer.name}</button>`;
                                    } else if (producer.status === 'missing') {
                                        chainHtml = `<button class="sf-ps-nav-source-btn" data-matid="${rm.id}" style="background:rgba(231,76,60,0.2);color:#e74c3c;font-size:10px;padding:2px 4px;border-radius:3px;margin-right:4px;border:1px solid #e74c3c;cursor:pointer;" title="انقر لفتح المتجر (مفقودة)">❗️ ${producer.name}</button>`;
                                    } else if (producer.status === 'animal_placed') {
                                        chainHtml = `<button class="sf-ps-nav-source-btn" data-matid="${rm.id}" style="background:rgba(230,126,34,0.8);color:#fff;font-size:10px;padding:2px 4px;border-radius:3px;margin-right:4px;border:none;cursor:pointer;" title="انقر للذهاب للحيوان">🐄 ${producer.name}</button>`;
                                    } else if (producer.status === 'animal') {
                                        chainHtml = `<button class="sf-ps-nav-source-btn" data-matid="${rm.id}" style="background:rgba(230,126,34,0.2);color:#e67e22;font-size:10px;padding:2px 4px;border-radius:3px;margin-right:4px;border:1px solid #e67e22;cursor:pointer;" title="انقر لفتح المتجر (حيوان)">🐄 ${producer.name}</button>`;
                                    } else if (producer.status === 'tree') {
                                        chainHtml = `<button class="sf-ps-nav-source-btn" data-matid="${rm.id}" style="background:rgba(39,174,96,0.2);color:#27ae60;font-size:10px;padding:2px 4px;border-radius:3px;margin-right:4px;border:1px solid #27ae60;cursor:pointer;" title="انقر لفتح المتجر (شجرة)">🌳 ${producer.name}</button>`;
                                    } else if (producer.status === 'seed') {
                                        chainHtml = `<button class="sf-ps-nav-source-btn" data-matid="${rm.id}" style="background:rgba(39,174,96,0.15);color:#2ecc71;font-size:10px;padding:2px 4px;border-radius:3px;margin-right:4px;border:1px solid #2ecc71;cursor:pointer;" title="انقر لفتح المتجر (محصول)">🌱 ${producer.name}</button>`;
                                    }
                                }
    ...
    // ═══════════════════════════════════════
    // STATUS BADGES (on map, read-only)
    // ═══════════════════════════════════════
    ...
    _positionBadges() {
        ...
        Object.keys(this.badges).forEach(key => {
            ...
            try {
                ...
                // Show if pinned, if machine is actively managed in UI, or if mouse is hovering nearby
    ...
    _findMachineProducing(productId) {
        ...
        for (let i = 0; i < this.items.length; i++) {
            let itm = this.items[i];
            if (itm.type === 'Animal' && String(itm.productId) === String(productId)) {
                return { status: 'animal_placed', name: itm.name, key: itm.key };
            }
        }
            ...
            if (gw.Config && gw.Config.originData) {
                ...
                for (let k in items) {
                    ...
                    if (itemType === 'machines') {
                        return { status: 'missing', name: `⚙️ ${itemName}`, id: cd.id };
                    } else if (itemType === 'animals' || subType === 'working') {
                        return { status: 'animal', name: `🐄 ${itemName}`, id: cd.id };
                    } else if (itemType === 'trees') {
                        return { status: 'tree', name: `🌳 ${itemName}`, id: cd.id };
                    } else if (itemType === 'seeds' || itemType === 'crops') {
                        return { status: 'seed', name: `🌱 ${itemName}`, id: cd.id };
                    } else {
                        return { status: 'missing', name: `❓ ${itemName}`, id: cd.id };
                    }
                }
            }
        } catch(e) {}
        return { status: 'missing', name: '❓ مصدر غير معروف', id: null };
    }
    ...
    _addChainToQueue(key, prodIdx, targetQty) {
        ...
        // Auto-start all dependent machines and animals that got queued
        dependentKeys.forEach(depKey => {
            const s = this.schedules[depKey];
            if (s) {
                if (s.queue && s.queue.length > 0) this._startOne(depKey);
                else if (s.targetCycles !== undefined) this._startOne(depKey);
            }
        });
        ...
        productConfig.reqMats.forEach(rm => {
            ...
            if (producer && producer.status === 'placed') {
                ...
                if (pItem) {
                    ...
                    if (childProdIdx !== -1) {
                        const sched = this.schedules[producer.key];
                        if (sched && sched.running && sched.queue && sched.queue.length > 0) {
                            const cur = sched.queue[sched.currentQueueIdx];
                            if (cur && cur.productIndex !== childProdIdx) {
                                this._log(`⚠️ الآلة ${producer.name} مشغولة بإنتاج "${cur.name}". تمت الإضافة للطابور!`);
                            }
                        }
            ...
            } else if (producer && producer.status === 'animal_placed') {
                const aItem = this.items.find(i => i.key === producer.key);
                if (aItem) {
                    const sched = this.schedules[producer.key] = this.schedules[producer.key] || { running: false, completedCycles: 0, targetCycles: 0 };
                    if (sched.running && sched.targetCycles === 0) {
                        this._log(`⚠️ الحيوان ${producer.name} يعمل مسبقاً باستمرار. تمت مشاركته في السلسلة.`);
                    }
                    sched.targetCycles += (targetQty === 0 ? 0 : targetQty);
                    this._log(`🔗 جدولة ذكية: تمت جدولة ${targetQty || '∞'} لحيوان ${producer.name}`);
                    if (!resultKeys.includes(producer.key)) resultKeys.push(producer.key);
                    
                    if (aItem.reqMats && aItem.reqMats.length > 0) {
                        this._collectDependencyKeys({ reqMats: aItem.reqMats }, targetQty, resultKeys);
                    }
                }
            } else if (producer && producer.status === 'missing') {
                this._log(`❗️ السلسلة ناقصة: ${producer.name} غير موجودة لإنتاج ${rm.name}.`);
            } else if (producer && (producer.status === 'animal' || producer.status === 'animal_placed')) {
                this._log(`🐄 السلسلة: ${rm.name} ينتجها ${producer.name} (حيوان غير موجود).`);
    ...
    _navigateByMaterialId(matId) {
        ...
        if (producer.status === 'placed' || producer.status === 'animal_placed') {
            ...
            if (item) {
                // If it's a machine, switch active tab to it
                if (item.type === 'Machine') {
