...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _checkAll() {
        ...
        const STAGGER_MS = Math.min(400, Math.floor((this.loopSpeed - 500) / Math.max(keys.length, 1)));
        keys.forEach((key, idx) => {
            setTimeout(() => {
                try { this._processItem(key); } catch(e) { this._log(`❌ ${key}: ${e.message}`); }
                // flush وsave بعد آخر عنصر فقط — دفعة واحدة لكل الأوامر
                if (idx === keys.length - 1) {
                    try { unsafeWindow.NetUtils.flush(); } catch(e) {}
                    this._saveSchedules();
                    this._checkAllBusy = false; // فتح القفل للدورة التالية
                }
            }, idx * STAGGER_MS);
        });
    }
