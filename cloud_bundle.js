...
SF.NetworkInterceptor = class NetworkInterceptor {
    ...
    _interceptXHR() {
        ...
        originalXHR.prototype.send = function(body) {
            ...
            // Safest 0-byte bug fix: Convert Uint8Array to ArrayBuffer (NO BLOB) to perfectly preserve Content-Type and avoid CORS!
            if (isGame && body && body instanceof Uint8Array) {
                arguments[0] = body.buffer;
            }
                ...
                if (pItem) {
                    if (pItem.type === 'Machine') {
                        ...
                        if (applyToQueue) {
                            ...
                            // Add target to the animal
                            this.schedules[producer.key].targetCycles += targetQty;
                        }
                        if (!resultKeys.includes(producer.key)) resultKeys.push(producer.key);
