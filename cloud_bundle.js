...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _findAllProducers(productId) {
        ...
        for (let i = 0; i < this.items.length; i++) {
            ...
            if (isMatch) {
                if (!placedDict[itm.id]) placedDict[itm.id] = { 
                    ...
                    type: itm.type,
                    name: itm.name, keys: [], id: itm.id, productIndex: pIdx
                };
        ...
        Object.values(placedDict).forEach(group => {
            producers.push({ status: group.status, type: group.type, name: group.name, key: group.keys[0], id: group.id, productIndex: group.productIndex });
        });
