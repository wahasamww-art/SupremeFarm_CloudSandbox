...
SF.BattlePassModule = class BattlePassModule extends SF.ModuleBase {
    ...
    executeSmartExploit() {
        ...
        let eventId = typeof model.getEvent === 'function' ? model.getEvent() : (model.data ? model.data.event : "");
        const netCore = gw.NetUtils || (gw.App && gw.App.NetUtils) || (gw.GF && gw.GF.NetUtils);

        // 1. تحديد الجوائز المتبقية ديناميكياً من الحدث الحالي (بدل 45 ثابتة)
        let payloadList = [];
        ...
        let payload = { action: "getReward", event: eventId, list: payloadList };
        this.logStatus(`🎁 جاري حصد ${payloadList.length} هدية لحدث ${eventId}...`);

        // 2. إرسال الطلب للسيرفر (يدعم الـ Promise)
        if (netCore && typeof netCore.request === 'function') {
            const req = netCore.request(model.method || "Activity/NewBattlePass", payload);
            if (req && typeof req.then === 'function') {
                req.then((res) => {
                    this._forceUnlockAndVisuals(model, payloadList, res, exactRewardsArray, gw);
                    this.logStatus("✅ تم حصد الجوائز بنجاح لحظياً!");
                }).catch(() => this.logStatus("⚠️ خطأ في الاتصال."));
            } else {
                // Fallback لو مفيش Promise
                this._forceUnlockAndVisuals(model, payloadList, null, exactRewardsArray, gw);
            }
        } else if (netCore && typeof netCore.enqueue === 'function') {
            netCore.enqueue(model.method || "Activity/NewBattlePass", payload);
            setTimeout(() => {
                this._forceUnlockAndVisuals(model, payloadList, null, exactRewardsArray, gw);
                this.logStatus("✅ تمت العملية بنجاح!");
            }, 1000);
        }
    }
    ...
    _forceUnlockAndVisuals(model, payloadList, res, exactRewardsArray, gw) {
        try {
            if (model.data) {
                if (!Array.isArray(model.data.reward)) model.data.reward = [];
                payloadList.forEach(id => {
                    if (!model.data.reward.includes(id)) {
                        model.data.reward.push(id);
                    }
                });
            }

            if (gw.GF && gw.GF.newBattlePassController && gw.GF.newBattlePassController.mainView && gw.GF.newBattlePassController.mainView.milestoneView) {
                gw.GF.newBattlePassController.mainView.milestoneView.udpateBP();
            }

            // عرض الهدايا في منتصف الشاشة لحظياً بالطريقة الأصلية للعبة
            if (exactRewardsArray.length > 0) {
                if (gw.GF && gw.GF.mysticTreasureHuntController && typeof gw.GF.mysticTreasureHuntController.openBpRewardView === 'function') {
                    gw.GF.mysticTreasureHuntController.openBpRewardView({
                        rwd: exactRewardsArray,
                        add: true
                    });
                } else {
                    // تحويل كائنات الهدايا إلى نص مفهوم في حال عدم توفر الدالة الأصلية
                    let strArray = exactRewardsArray.map(r => r.id + ":" + r.qty);
                    this.extractAndPlayVisuals(strArray.join(","));
                }
            }
        } catch (e) {
            console.error("[SF-BattlePass] Error in visuals:", e);
        } finally {
...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _positionBadges() {
        ...
        // ✅ إخفاء panel الجدولة عند فتح نافذة معلومات/كائن في اللعبة
    ...
    _checkAll() {
        ...
        // Failsafe: لو القفل عالق أكثر من 10 ثوان ← فكّه بالقوة
        if (this._checkAllBusy) {
            if (this._checkAllBusySince && Date.now() - this._checkAllBusySince > 10000) {
                this._log('⚠️ _checkAllBusy stuck > 10s — force unlocking');
        ...
        const allKeys = Object.keys(this.schedules).filter(k => this.schedules[k].running);
        if (allKeys.length === 0) { this._checkAllBusy = false; this._checkAllBusySince = 0; this._stopLoop(); return; }

        // Rate Limiting: 3 عناصر فقط (cumulative delay max ≈ 2400ms < loopSpeed 3000ms)
        const MAX_PER_CYCLE = 3;
        if (!this._checkAllOffset) this._checkAllOffset = 0;
        if (this._checkAllOffset >= allKeys.length) this._checkAllOffset = 0;
        const keys = allKeys.slice(this._checkAllOffset, this._checkAllOffset + MAX_PER_CYCLE);
        this._checkAllOffset += keys.length;
        ...
        keys.forEach((key, idx) => {
            ...
            setTimeout(() => {
                ...
                if (idx === keys.length - 1) {
                    // لا نستدعي NetUtils.flush() — اللعبة ترسل الطلبات تلقائياً عبر AMF queue
    ...
    _sendZeroGasRefill(mo, matId, slot) {
        try {
            ...
            if (gw.NetUtils && typeof gw.NetUtils.enqueue === 'function') {
                const sd = mo.serverData || {};
                const payload = {
                    id: Number(mo.id || mo.configData?.id),
                    x: Number(parseInt(sd.x || sd.map_x) || 0),
                    y: Number(parseInt(sd.y || sd.map_y) || 0),
                    flip: mo.flip ? 1 : 0,
                    material: Number(matId),
                ...
                gw.NetUtils.enqueue("refill_object.save_data", payload);
    ...
    _sendZeroGasFeed(mo, matId) {
        try {
            ...
            if (gw.NetUtils && typeof gw.NetUtils.enqueue === 'function') {
                ...
                delete payload.isAuto; delete payload.automatic; delete payload.is_auto; delete payload.op_cost; delete payload.use_op;
                gw.NetUtils.enqueue("feed_object.save_data", payload);
