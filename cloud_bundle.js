...
SF.SplashScreen = class SplashScreen {
    ...
    show(callback) {
        ...
        // ✅ نستنى اللعبة تتحمل فعلاً (uidDictionary يتملأ) بدل timer ثابت
        const MAX_WAIT_MS   = 15000; // حد أقصى 15 ثانية
        const POLL_INTERVAL = 500;   // فحص كل 500ms
        const MIN_SHOW_MS   = 2000;  // أقل مدة للظهور حتى لو اللعبة سريعة
        let elapsed = 0;
        ...
        const checkReady = setInterval(() => {
            ...
            try {
                ...
                isReady = dict && Object.keys(dict).length > 0;
            } catch(e) {}

            const shownEnough = (Date.now() - startTime) >= MIN_SHOW_MS;

            if ((isReady && shownEnough) || elapsed >= MAX_WAIT_MS) {
...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _positionBadges() {
        ...
        badgeKeys.forEach(key => {
            ...
            try {
                ...
                positions[key] = { sx: rect.left + p.x * scX, sy: rect.top + p.y * scY - 20 };
    ...
    _applyHoverFromCache(mx, my) {
        ...
        // احسب أقرب badge من الماوس
        let closestKey = null;
        let closestDist = Infinity;
        const HIT_RADIUS = 55; // بكسل — نطاق الـ hit detection على الآلة/الحيوان

        Object.keys(positions).forEach(key => {
            const pos = positions[key];
            if (!pos) return;
            const dist = Math.hypot(pos.sx - mx, pos.sy - my);
        ...
        // ✅ Phase 2 (DOM writes): أظهر الأقرب فقط، أخفِ الباقي
        Object.keys(this.badges).forEach(key => {
            const badge = this.badges[key];
            if (!badge) return;
            const pos = positions[key];
            ...
            if ((isNearest || isPinned || isActive) && sx > rect.left - 30 && sx < rect.right + 30 && sy > rect.top - 30 && sy < rect.bottom + 30) {
