...
SF.NetworkInterceptor = class NetworkInterceptor {
    ...
    _interceptXHR() {
        ...
        originalXHR.prototype.send = function(body) {
            ...
            // Restored Blob conversion without 'type' to fix 0-byte bug AND prevent CORS issues
            if (isGame && body && (body instanceof ArrayBuffer || body instanceof Uint8Array)) {
                arguments[0] = new Blob([body]);
            }
...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    render() {
        ...
        <div class="sf-card" style="padding: 20px; display:flex; flex-direction:column; height: 100%; box-sizing: border-box;">
            ...
            <div style="display:flex; flex-direction:row; gap:10px; flex:1; overflow:hidden;">
                <div id="sf-ps-view-machines" style="flex:1; display:flex; flex-direction:column; background:rgba(0,0,0,0.2); border-radius:8px; padding:8px; border:1px solid #333;">
                ...
                <div id="sf-ps-view-animals" style="flex:1; display:flex; flex-direction:column; background:rgba(0,0,0,0.2); border-radius:8px; padding:8px; border:1px solid #333;">
                        ...
                        if (producer && (producer.status === 'placed' || producer.status === 'animal_placed')) {
                            ...
                            if (prodSched) {
                                ...
                                if (!prodSched.running) {
                                    prodSched.running = true;
                                    changed = true;
                                    this._log(`🔗 السلسلة الذكية: تم تشغيل آلة العلف.`);
                                }
                                if (changed) {
                                    this._saveSchedules();
                                    this._renderMachines();
