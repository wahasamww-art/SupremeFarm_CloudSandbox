...
// ══════════════════════════════════════════════════════════
// نظام حماية الأدمن — كلمة سر عند الضغط على التبويب
// ══════════════════════════════════════════════════════════
SF._AdminGuard = {
    SECRET:    'admin',
    _unlocked: false,
    ...
    // يُستدعى عند الضغط على تبويب adminOnly
    // callback(true) = صح | callback(false) = خطأ
    requestAccess(callback) {
        if (this._unlocked) { callback(true); return; }

        // نافذة كلمة السر المخصصة
        const overlay = document.createElement('div');
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999999;display:flex;align-items:center;justify-content:center;';

        const box = document.createElement('div');
        box.style.cssText = 'background:#1a1a2e;border:1px solid #e74c3c;border-radius:14px;padding:28px 32px;min-width:300px;text-align:center;box-shadow:0 0 40px rgba(231,76,60,.3);';
        box.innerHTML = `
            <div style="font-size:32px;margin-bottom:8px">🔐</div>
            <div style="color:#fff;font-size:16px;font-weight:bold;margin-bottom:6px">منطقة محمية</div>
            <div style="color:#aaa;font-size:12px;margin-bottom:16px">أدخل كلمة السر للمتابعة</div>
            <input id="sf-admin-input" type="password" placeholder="كلمة السر..."
                style="width:100%;box-sizing:border-box;padding:10px 14px;border-radius:8px;border:1px solid #444;background:#0d0d1a;color:#fff;font-size:14px;text-align:center;outline:none;margin-bottom:10px;">
            <div id="sf-admin-err" style="color:#e74c3c;font-size:12px;min-height:16px;margin-bottom:10px;"></div>
            <div style="display:flex;gap:10px;">
                <button id="sf-admin-cancel" style="flex:1;padding:9px;border-radius:8px;border:none;background:#333;color:#aaa;cursor:pointer;font-size:13px;">إلغاء</button>
                <button id="sf-admin-ok"     style="flex:1;padding:9px;border-radius:8px;border:none;background:#e74c3c;color:#fff;cursor:pointer;font-size:13px;font-weight:bold;">دخول</button>
            </div>
        `;
        ...
        const input  = box.querySelector('#sf-admin-input');
        const errEl  = box.querySelector('#sf-admin-err');
        const okBtn  = box.querySelector('#sf-admin-ok');
        const cancel = box.querySelector('#sf-admin-cancel');

        setTimeout(() => input.focus(), 50);

        const attempt = () => {
            if (input.value === this.SECRET) {
                this._unlocked = true;
                sessionStorage.setItem(this._STORE_KEY, '1');
                overlay.remove();
                callback(true);
            } else {
                ...
                setTimeout(() => { input.style.borderColor = '#444'; errEl.textContent = ''; }, 2000);
            }
        };

        okBtn.onclick   = attempt;
        cancel.onclick  = () => { overlay.remove(); callback(false); };
        input.onkeydown = (e) => { if (e.key === 'Enter') attempt(); if (e.key === 'Escape') { overlay.remove(); callback(false); } };
        overlay.onclick = (e) => { if (e.target === overlay) { overlay.remove(); callback(false); } };
    },

    lock() {
        this._unlocked = false;
        sessionStorage.removeItem(this._STORE_KEY);
    }
};
...
SF.UIManager = class UIManager {
    constructor() {
        ...
        const firstMod = SF.modules.getModules().find(m => !m.adminOnly);
        if (firstMod) {
    ...
    createLayout() {
        ...
        this.mainMenuBtn.onclick = () => this.dropdownMenu.classList.toggle('sf-hidden');
    ...
    addModuleTab(mod) {
        ...
        tab.id = `tab-${mod.id}`;
        // أيقونة قفل بجانب اسم الوحدات المحمية
        tab.innerHTML = mod.adminOnly
            ? `${mod.icon} <span>${mod.name}</span> <span style="font-size:10px;opacity:.7">🔒</span>`
            : `${mod.icon} <span>${mod.name}</span>`;

        tab.onclick = () => {
            this.dropdownMenu.classList.add('sf-hidden');

            if (mod.adminOnly && !SF._AdminGuard.isUnlocked()) {
                // طلب كلمة السر قبل الفتح
                SF._AdminGuard.requestAccess((granted) => {
                    if (!granted) return;
                    // بعد نجاح كلمة السر: نحدّث أيقونة القفل ونفتح
                    tab.innerHTML = `${mod.icon} <span>${mod.name}</span> <span style="font-size:10px;opacity:.7">🔓</span>`;
    ...
    switchTab(moduleId) {
        ...
        if (activeMod) this.titleEl.innerHTML = `${activeMod.icon} ${activeMod.name}`;
    }
};
...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    bindEvents() {
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            // ✅ Instant hover: تحقق فوراً من الـ cache وظهّر/أخفِ الكارت لحظياً
    ...
    _checkAll() {
        ...
        // ✅ معالجة كل عنصر بفاصل 400ms بدلاً من forEach متزامن
        // السبب: كل collect يستدعي checkDropObjByArrayOnTree2015 في اللعبة
        // وهي دالة ثقيلة — تشغيلها 7 مرات متتالية = قفشة مضمونة
        const STAGGER_MS = 400;
        keys.forEach((key, idx) => {
            setTimeout(() => {
                try { this._processItem(key); } catch(e) { this._log(`❌ ${key}: ${e.message}`); }
                // flush وsave بعد آخر عنصر فقط
