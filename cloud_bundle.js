...
SF.NetworkInterceptor = class NetworkInterceptor {
    ...
    _interceptXHR() {
        ...
        originalXHR.prototype.send = function(body) {
            ...
            // GEMINI.md Protocol: Convert to Blob to fix 0-byte bug in some browsers
            // Content-Type is already set by AMF client via setRequestHeader — do NOT set type here to avoid CORS preflight
            if (isGame && body && (body instanceof Uint8Array || body instanceof ArrayBuffer)) {
                arguments[0] = new Blob([body instanceof Uint8Array ? body.buffer : body]);
            }
...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _stopLoop() { if (this.loopTimer) { clearInterval(this.loopTimer); this.loopTimer = null; } this._checkAllBusy = false; }
    ...
    _checkAll() {
        ...
        if (allKeys.length === 0) { this._checkAllBusy = false; this._stopLoop(); return; }

        // Rate Limiting: معالجة 5 عناصر فقط في كل دورة لمنع إغراق السيرفر
