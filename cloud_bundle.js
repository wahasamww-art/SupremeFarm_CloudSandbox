...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _collectGhost(uid) {
        ...
        // Mark as collected instead of deleting, so the user can optionally return it
        g.isCollected = true;
        this._saveSchedules();
        
        alert(`تم جمع ${g.qty} من ${g.productName} بنجاح وإضافتها للمستودع! الآلة الآن في حالة خمول داخل مركز الأشباح.`);
        this._renderGhosts();
    }
