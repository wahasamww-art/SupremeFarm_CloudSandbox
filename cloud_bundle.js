...
SF.CustomBackgroundModule = class CustomBackgroundModule {
    ...
    injectBackground() {
        // 1. Inject CSS for common background containers
        const style = document.createElement('style');
        style.textContent = `
        ...
        // 2. Intercept Image loading to catch canvas-drawn backgrounds
        ...
        if (originalImageSrc) {
            ...
            Object.defineProperty(Image.prototype, 'src', {
                set: function(val) {
                    let newVal = val;
                    if (typeof val === 'string' && val.toLowerCase().includes('.jpg')) {
                        if (val.toLowerCase().includes('login') || val.toLowerCase().includes('loading') || val.toLowerCase().includes('splash') || val.toLowerCase().includes('bg_loading')) {
                            newVal = self.base64Img;
                            console.log('[SupremeFarm] Intercepted background image:', val);
                        }
                    }
