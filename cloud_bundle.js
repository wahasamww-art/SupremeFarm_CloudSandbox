...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _checkAll() {
        ...
        if (keys.length === 0) { this._checkAllBusy = false; this._stopLoop(); return; }

        // حساب ديناميكي للفاصل الزمني — يضمن عدم تجاوز loopSpeed
        // مع 4 ماكينات: min(400, floor(2500/4)) = 400ms ← مجموع = 1200ms (آمن)
        // مع 8 ماكينات: min(400, floor(2500/8)) = 312ms ← مجموع = 2184ms (آمن)
        // مع 15 ماكينة: min(400, floor(2500/15)) = 166ms ← مجموع = 2324ms (آمن)
        const BASE_STAGGER = Math.min(400, Math.floor(Math.max(this.loopSpeed - 500, 200) / Math.max(keys.length, 1)));
        ...
        keys.forEach((key, idx) => {
            ...
            cumulativeDelay += BASE_STAGGER + jitter;
            setTimeout(() => {
                try {
