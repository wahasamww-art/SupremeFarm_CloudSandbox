...
SF.CustomBackgroundModule = class CustomBackgroundModule {
    ...
    injectBackground() {
        ...
        const isBgUrl = (url) => {
            ...
            // Match common splash/loading background names, but avoid UI atlases
            if (u.includes('.jpg') || u.includes('.png')) {
                if (u.includes('loading') || u.includes('login') || u.includes('splash') || 
                    u.includes('bg_') || u.includes('bg.') || u.includes('cover') || u.includes('scene4_bg')) {
                    // Exclude known UI atlases or small icons if they contain these words
                    if (u.includes('atlas') || u.includes('sheet') || u.includes('icon') || u.includes('btn') || u.includes('ui')) {
                        return false;
                    }
                    return true;
                }
        ...
        // 3. Intercept XHR open to rewrite URLs
        ...
        // 4. Intercept Fetch to rewrite URLs
        if (window.fetch) {
