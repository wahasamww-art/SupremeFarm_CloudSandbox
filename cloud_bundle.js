...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _updateBadge(key) {
        ...
        } else if (p && typeof p.nextProductTime === 'function' && p.nextProductTime() > 0) {
            const remain = p.nextProductTime();
            const mins = Math.floor(remain / 60);
            const secs = remain % 60;
            reportHtml += `<div>⏳ متبقي: <span style="color:#f1c40f">${mins}:${secs.toString().padStart(2, '0')}</span></div>`;
        } else {
            reportHtml += `<div>💤 حالة الإنتاج: <span style="color:#95a5a6">خامل / قيد الانتظار</span></div>`;
        }
    ...
    _processItem(key) {
        ...
        // Skip calling game APIs every 3 seconds if the machine is still busy
        if (typeof mo.nextProductTime === 'function') {
            const remain = mo.nextProductTime();
            if (remain > 0) {
                this._updateBadge(key); // Update countdown visually
                return; // Machine/Animal is busy, skip hitting game APIs
            }
        }
