            ...
            if (next && next.productIndex >= 0) {
                ...
                try {
                    ...
                    if (p && p.reqMats && p.reqMats.length > 0 && gc._refillMapObject) {
                        // تعبئة كل خانة بتأخير عشوائي لمنع إرسال دفعة واحدة
                        p.reqMats.forEach((rm, s) => {
                            setTimeout(() => {
                                try { gc._refillMapObject(mo, rm.id, s + 1, false); } catch(e) {}
                            }, s * (Math.floor(Math.random() * 500) + 500));
                        });
                        this._updateBadge(key);
                        this._log(`▶ بدء ${item.name}: ${next.name}`);
                        setTimeout(() => this._processItem(key), p.reqMats.length * 1000 + 500);
                    } else {
                        const slot = 1;
