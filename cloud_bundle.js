...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
    ...
    _renderAnimals() {
        ...
        const renderAnimal = (item) => {
            ...
            const iconHtml = this._getItemIconUrl(item.id, '🐄', 24);
    ...
    _renderMachines() {
            ...
            <div class="sf-ps-manage-view" data-key="${item.key}" style="background:rgba(52,152,219,0.05);border:1px solid #3498db44;border-radius:8px;padding:12px;box-shadow:inset 0 0 10px rgba(0,0,0,0.5);">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;border-bottom:1px solid #333;padding-bottom:8px;">
                    ...
                    <span style="color:#3498db;font-size:16px;font-weight:bold;display:flex;align-items:center;gap:6px;">
                        ${this._getItemIconUrl(item.id, '⚙️', 24)}
                        ${item.name}
                    </span>
                </div>
                ...
                <div style="margin-top:12px;border-top:1px solid #333;padding-top:10px;">
                    ...
                    <div class="sf-ps-prod-list" style="max-height:150px;overflow-y:auto;padding-right:4px;">
                        ${item.products.map((p, i) => {
                            ...
                            const prodIconHtml = p.productId ? this._getItemIconUrl(p.productId, '📦', 22) : '';
        ...
        const renderMachine = (item) => {
            ...
            const queue = sched?.queue || [];
            const isFav = !!this.favorites[item.key];
            const iconHtml = this._getItemIconUrl(item.id, '⚙️', 24);
