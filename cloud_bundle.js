...
SF.MergeAutoPlayModule = class extends SF.ModuleBase {
    ...
    doOrders(model, mapDict) {
        // Read orders from memory dump structure
        ...
        for (let order of orders) {
            ...
            // Order requirements (e.g. "91011" or "91022,91015")
            const neededIds = order.item.split(',').map(s => Number(s));
            let foundTags = [];
            
            // Search the board for the needed items
            for (let reqId of neededIds) {
                ...
                for (let key in mapDict) {
                    if (key.match(/^\d+_\d+$/)) {
                        ...
                        // Item must exist, match ID, and NOT be locked (lock: 1 means frozen in ice)
            ...
            // If we found ALL required items on the board, the order is ready!
            if (foundTags.length === neededIds.length) {
                this.log(`جاري تسليم الطلب! العناصر: ${neededIds.join(', ')}`);
                
                // Submit order: some versions use action:"use" on the items, others use "finish_order".
                // We will send finish_order with the order index, which is the standard backend command!
                ...
                if (gw.NetUtils && gw.NetUtils.enqueue) {
                    gw.NetUtils.enqueue(endpoint, {
                        ...
                        this.log(`تم تأكيد تسليم الطلب ${order.index} بنجاح!`);
                    });
                }
                
                // Also track the consumed items so we don't try to merge them before they disappear
                for (let tag of foundTags) {
                    this.pendingMerges.add(tag);
                }
                
                orderSubmitted = true;
                return; // Do one order per tick to prevent out-of-sync
            }
        }
        ...
        if (!orderSubmitted) {
            ...
            // Fallback to merging to build the required items!
            this.doMergeAll(mapDict);
        }
    }
