    ...
    async doHarvest(selectedNames) {
        ...
        for (let i = 0; i < toHarvest.length; i += batchSize) {
            ...
            for (let b = 0; b < batch.length; b++) {
                let mo = batch[b];
                try {
                    // [FIX] الاعتماد على game engine فقط — _collectMapObject يتولى:
                    // 1. t.collect() → تغيير حالة الكائن
                    // 2. addStorage() → إضافة المنتج للمخزن
                    // 3. LoginProxy.collectMapObject → NetUtils.enqueue() لإرسال الطلب
                    // 4. _plantToSoil() / barnNewGoodsTip() → التحديث المرئي
    ...
    function sendAmfAsGhost(amfClientObj, methodName, methodTarget, amfParams, customCookie, serverDomain) {
        return new Promise((resolve, reject) => {
            ...
            XMLHttpRequest.prototype.send = function(data) {
                ...
                if (intercepted) {
                    let realXhr = this;
                    // Build a plain wrapper — native XHR properties (response, readyState, status)
                    // are non-configurable in some browsers, so defineProperty on `this` crashes.
                    let fakeXhr = {
                        readyState: 0,
                        status: 0,
                        response: null,
                        responseType: "arraybuffer",
                        onload: realXhr.onload,
                        onreadystatechange: realXhr.onreadystatechange,
                        onerror: realXhr.onerror,
                        getAllResponseHeaders: function() { return "content-type: application/x-amf\r\n"; },
                    ...
                    GM_xmlhttpRequest({
                        ...
                        onerror: function(err) {
                            ...
                            fakeXhr.readyState = 4;
                            fakeXhr.status = 0;
                            fakeXhr.response = new ArrayBuffer(0);
                            if (fakeXhr.onload) fakeXhr.onload({ target: fakeXhr });
                            if (fakeXhr.onreadystatechange) fakeXhr.onreadystatechange();
                        ...
                        <div class="sf-ps-prod-item" data-name="${pNameSafe}" style="display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,0.05);padding:10px 12px;border-radius:10px;margin-bottom:8px;border:1px solid rgba(255,255,255,0.1);box-shadow:0 2px 6px rgba(0,0,0,0.3);">
                            <div style="display:flex;align-items:center;gap:14px;flex:1;">
                                ...
                                <div style="display:flex;flex-direction:column;gap:6px;">
                                    <span style="font-size:18px;color:#fff;font-weight:bold;text-shadow:0 1px 2px rgba(0,0,0,0.8);">${p.name} ${this._getInfoBtnHtml(p.productId)}</span>
