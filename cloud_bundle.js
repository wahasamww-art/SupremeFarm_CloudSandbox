    ...
    injectHook() {
        const checkAndHook = () => {
            ...
            if (gw.NetUtils && gw.NetUtils.enqueue && !gw.NetUtils._sf_hooked) {
                ...
                gw.NetUtils.enqueue = function(payload, successCb, failCb) {
                    if (payload && payload.action && !payload.action.includes('login.login') && !payload.action.includes('activity.check') && !payload.action.includes('friend.get')) {
                        SF.bus.emit('amf:intercepted', { action: payload.action, params: payload.params });
                    }
