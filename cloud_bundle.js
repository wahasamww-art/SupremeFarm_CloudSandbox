...
SF.CustomBackgroundModule = class CustomBackgroundModule {
    ...
    injectBackground() {
        ...
        const isBgUrl = (url) => {
            ...
            if (u.includes('.jpg') || u.includes('.png') || u.includes('.webp') || u.includes('.jpeg')) {
                if (u.includes('loading') || u.includes('login') || u.includes('splash') || 
                    u.includes('bg_') || u.includes('bg.') || u.includes('cover') || u.includes('scene4_bg') || u.includes('background') || u.includes('start') || u.includes('first')) {
                    // Exclude known UI atlases or small icons if they contain these words
                    if (u.includes('atlas') || u.includes('sheet') || u.includes('icon') || u.includes('btn') || u.includes('ui_') || u.includes('font')) {
                        return false;
                    }
                    return true;
                }
        ...
        // Exploratory Probing: Find the exact background image URL
        const probeDiv = document.createElement('div');
        probeDiv.style.cssText = 'position:fixed;top:10px;left:10px;background:rgba(0,0,0,0.8);color:#0f0;z-index:999999;padding:10px;font-family:monospace;max-height:90vh;overflow-y:auto;direction:ltr;';
        probeDiv.innerHTML = '<h3>[Probing] Large Images:</h3><ul id="sf-probe-list"></ul>';
        
        if (document.body) document.body.appendChild(probeDiv);
        else document.addEventListener('DOMContentLoaded', () => document.body.appendChild(probeDiv));

        const addProbeUrl = (url, source, w = 0, h = 0) => {
            if (!url || typeof url !== 'string' || url.startsWith('data:')) return;
            const list = document.getElementById('sf-probe-list');
            if (list && !list.innerHTML.includes(url)) {
                const li = document.createElement('li');
                li.textContent = `[${source}] ${w}x${h} : ${url.split('/').pop()}`;
                li.style.cursor = 'pointer';
                li.onclick = () => prompt('Copy URL:', url);
                list.appendChild(li);
            }
        };

        // Probe 1: Performance Observer for Resource Timing
        if (window.PerformanceObserver) {
            const observer = new PerformanceObserver((list) => {
                list.getEntries().forEach(entry => {
                    if (entry.initiatorType === 'img' || entry.name.match(/\.(png|jpg|jpeg|webp)/i)) {
                        addProbeUrl(entry.name, 'Network');
                    }
                });
            });
        ...
        // Probe 2: DOM Mutation Observer for img tags
        const domObserver = new MutationObserver((mutations) => {
            mutations.forEach(m => {
                m.addedNodes.forEach(node => {
                    if (node.tagName === 'IMG') {
                        node.onload = () => {
                            if (node.naturalWidth > 500 || node.naturalHeight > 500) {
                                addProbeUrl(node.src, 'DOM-IMG', node.naturalWidth, node.naturalHeight);
                            }
                        };
                    }
                });
        ...
        if (document.body) {
            ...
            document.addEventListener('DOMContentLoaded', () => domObserver.observe(document.body, { childList: true, subtree: true }));
        }

        // Probe 3: XHR / Fetch logger
        const origXhrOpen = window.XMLHttpRequest.prototype.open;
        window.XMLHttpRequest.prototype.open = function(method, url) {
            if (typeof url === 'string' && url.match(/\.(png|jpg|jpeg|webp)/i)) {
                addProbeUrl(url, 'XHR');
            }
