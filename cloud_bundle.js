...
SF.SplashScreen = class SplashScreen {
    ...
    show(callback) {
        ...
        // ✅ 4 ثواني ثابتة — كافية لتغطي تحميل اللعبة دون إخفاء القطة لفترة طويلة
        setTimeout(() => {
...
SF.AutoFarmModule = class AutoFarmModule extends SF.ModuleBase {
    ...
    async doHarvest(selectedNames) {
        ...
        let count = 0;
        const batchSize = 50; // حجم دفعة آمن لمنع إغراق السيرفر
        for (let i = 0; i < toHarvest.length; i += batchSize) {
            ...
            await new Promise(r => setTimeout(r, 200)); // تأخير 200ms بين الدفعات لمحاكاة السلوك البشري ومنع rate-limiting
        }
    ...
    _processMachine(key, item, mo, sd, sched) {
        ...
        if (hasProducts) {
            ...
            try { 
                ...
                if (unsafeWindow.NetUtils && typeof unsafeWindow.NetUtils.flush === 'function') {
                    unsafeWindow.NetUtils.flush();
                }
            } catch(e) {}
