    ...
    scanHarvest() {
        ...
        try {
            ...
            moList.forEach(mo => {
                ...
                if (mo.house_id || sd.wid || sd.habitat) return;
...
SF.ProductionSchedulerModule = class ProductionSchedulerModule extends SF.ModuleBase {
                ...
                if (remain > 0) {
                    ...
                    if (!sched.__preciseTimer || sched.__preciseTimerTime !== remain) {
                        ...
                        sched.__preciseTimer = setTimeout(() => {
                            sched.__preciseTimer = null;
                            this._processItem(key);
                        }, (remain * 1000) + 4000); // 4s buffer to prevent server "not products" rejection
                    }
                    return; 
                }
            }
    ...
    _processMachine(key, item, mo, sd, sched) {
        ...
        if (hasProducts) {
            ...
            // [SERVER TIME GUARD] - Prevent clock-drift "not products" rejection.
            if (now - sched.__ready_timestamp < 10000) {
                setTimeout(() => this._processItem(key), 800);
                return;
            }

            if (sched.__last_collect_time && now - sched.__last_collect_time < 15000) {
                setTimeout(() => this._processItem(key), 800);
                return;
            }
