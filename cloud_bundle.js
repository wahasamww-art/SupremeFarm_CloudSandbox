...
SF.MergeAutoPlayModule = class extends SF.ModuleBase {
    ...
    doOrders(model, mapDict) {
        // Find orders array in the model
        let orders = model.orders || model.orderData || model.orderList || model.tasks || model.taskList;
        if (!orders) {
            // Some models store it deeper
            for (let k in model) {
                if (Array.isArray(model[k]) && model[k].length > 0 && model[k][0].hasOwnProperty('rewards')) {
                    orders = model[k];
                    break;
                }
            }
        ...
        for (let order of orders) {
            // If the order is marked as ready or we can verify its requirements
            // A simple approach: find items on the board that match the order requirements.
            // But since the game marks items when an order is ready, maybe the items have a flag!
            // Let's just look for any item in mapDict that has 'canUse' or similar if order structure is complex.
            
            // To be safe, if we know the action is "use" and the tag is the position:
            if (order.status === 1 || order.state === 1 || order.isComplete || order.canSubmit) {
                // If the model tells us it's ready, we need to find the items to use.
                // Actually, some games let you just send the "use" for the target item directly.
            }
        }
        
        // Alternative brute force (smart heuristic): scan mapDict for items that have `isOrder` or `canUse` set to true.
        for (let key in mapDict) {
            if (key.match(/^\d+_\d+$/)) {
                if (this.pendingMerges.has(key)) continue;
                const item = mapDict[key];
                
                // If the item itself has a flag indicating it can be used/submitted
                if (item && typeof item === 'object' && (item.canUse || item.isOrderReady || item.canSubmit || item.showOrderIcon)) {
                    this.sendAction("use", key, null);
        ...
        if (!orderSubmitted) {
            // this.log('لا يوجد طلبات جاهزة للتسليم حالياً.');
        }
    }
    ...
    doMergeAll(mapDict) {
        ...
        // Clean up pending if they are gone from the map
        ...
        for (let key in mapDict) {
            if (key.match(/^\d+_\d+$/)) {
                if (this.pendingMerges.has(key)) continue; // skip items we are currently merging
                
                const item = mapDict[key];
                if (item && typeof item === 'object' && item.id) {
                    if (item.isLock || item.locked) continue;
                    if (!itemsByLevel[item.id]) itemsByLevel[item.id] = [];
                    itemsByLevel[item.id].push(key);
                }
            }
        }

        for (let id in itemsByLevel) {
            const positions = itemsByLevel[id];
    ...
    bindEvents() {
        ...
        c.querySelector('#sf-dump-btn').addEventListener('click', () => {
            ...
            if (orders && orders.length > 0) {
                try {
                    sampleOrder = JSON.parse(JSON.stringify(orders[0], (key, val) => {
                        if (key.startsWith('_') || typeof val === 'function') return undefined;
                        return val;
                    }));
                } catch(e) {}
